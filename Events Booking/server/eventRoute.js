...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    cb(null, uploadPath); // ✅ Ensure correct path
  },
...
router.post('/uploadImage', upload.single('image'), (req, res) => {
  ...
  // Use the server's host dynamically
...
router.get('/eventViews/:eventID', async (req, res) => {
  try {
    ...
    let format = '%Y-%m-%d'; // default format
    ...
    // 🚨 Do not apply date filter if filter === 'all'
...
router.get('/eventSales/:eventID', async (req, res) => {
  try {
    ...
    if (filter) {
      ...
      // No filter = all tickets and no aggregation
