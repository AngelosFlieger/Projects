...
const EventViewsScreen = () => {
  ...
  useEffect(() => {
    fetchAllTickets(); // for demographics (one-time)
  }, []);

  useEffect(() => {
    fetchTotalViews(); // for total views — only once
  }, []);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#111" }}>
      ...
      <ScrollView
        ...
        style={{ marginTop: 10 }} // Adjust this based on your header height
      >
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            try {
              // 1. Mark event as finished
              await axios.patch(`${serverIP}/eventRoute/${event._id}/status`);

              // 2. Delete all tickets for the event
              await axios.delete(
