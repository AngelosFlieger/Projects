...
import { useNavigation, CommonActions } from "@react-navigation/native"; // ✅ Reset navigation
import { ProfileIcon, LogoutIcon, DeleteIcon } from "./components/Icons";

const { width: screenWidth } = Dimensions.get("window");

const UserInfoScreen: React.FC = () => {
  const { serverIP, user, setUser } = useAuth(); // ✅ Access logout logic
  ...
  const handleLogout = () => {
    Alert.alert("Logout", "Are you sure you want to log out?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Yes",
        onPress: () => {
          setUser(null); // ✅ Clears user data in context
          navigation.dispatch(
            CommonActions.reset({
              index: 0,
              routes: [{ name: "index" }], // ✅ Resets stack to prevent going back
            })
          );
        },
      },
